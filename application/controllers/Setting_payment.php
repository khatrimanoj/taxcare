...
class Setting_payment extends CI_Controller {
...
/*  		$data['module']=$this->Payment_model->getModule();
      $data['states']=$this->State_district_block_model->getStates();
      $settingsArray =$this->Payment_model->getSettings();*/
    ...
    { 
      // check user permission
      if(!has_admin_permission_layout('EDIT_SETTINGS')) { return; }
      $module=$this->Payment_model->getModule();

      $states= $this->State_district_block_model->getStates();
      
      $settingsArray =$this->Payment_model->getSettings($rid);
      extract($_POST);
      $records=array();
      ...
      $is_insert_successful = $this->Payment_model->emptytableByRound();
      ...
      foreach ($module as $mod) {
        foreach($states as $st){
            if(isset($_POST['module_id_'.$mod['module_id'].'_State_ID_'.$st['State_ID']]))
            {
             ...
             $is_insert_successful = $this->Payment_model->insertData($data);
    ...
    public function globalText(){
      ...
      $data['records']=$this->Payment_model->getGlobalText();
    ...
    public function saveGolbalText(){
      ...
      foreach($this->input->post('id') as $id) {
        ...
        $this->Payment_model->updateGlobalText($id,$data);
      }
