...
class Media_model extends MY_Model {
	...
	public function getPhotos($filterArray,$limit=0,$start=0){
		...
		$query = "SELECT SQL_CALC_FOUND_ROWS * FROM setting_social_media  WHERE " . implode(' AND ', $queryWhere). "";
	...
	public function get_last_calculated_total()
	{
     ...
     public function getGalleryList($filterArray,$per_page='', $offset=''){
		...
		$query    	= "select  *  from setting_social_media	";
...
//echo $query;die;
	...
	// get info for already edit
	   ...
	   return $this->db->get_where('setting_social_media',array('id'=>$id))->row();
	...
	public function active($id){
		...
		if($this->db->update("setting_social_media")){	
			return true;
		}else{
		   return true;
		}
	...
	public function checkStatus($id){
		...
		$this->db->select('*');
		$this->db->from("setting_social_media");
		$this->db->where('id',$id);
		$query = $this->db->get();
	...
	public function record_count_video(){
        $sql    =   $this->db->query("select * from `setting_social_media` where status='1' and publish='1' and type='2'");
        return $sql->num_rows();
    }
    public function fetch_data_video($limit, $start,$stateID,$districtID){
        $query  =   "select * from `setting_social_media` where 1  ";
	...
	//Fetch Photo Gallery For Front
      ...
      $sql    =   $this->db->query("select * from `setting_social_media` where status='1' and publish='1' and type='1'");
      return $sql->num_rows();
    }
  	...
  	public function update_banner($id,$data){
		...
		$this->db->where('id', $id);
		return $this->db->update('setting_social_media', $data);
		 //echo $this->db->last_query();exit;
		
	}
  	public function update_youtube($id,$data){
		
		$this->db->where('id', $id);
		return $this->db->update('setting_youtube', $data);
		 //echo $this->db->last_query();exit;
