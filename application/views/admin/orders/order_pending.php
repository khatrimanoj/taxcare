                                           ...
                                           <?php
                                            foreach ($orders as $order) { ?>
                                                <tr>
                                                    ...
                                                    <?php
                                                    if ($order['appl_status'] == 1) {
                                                        $status_value ='<span class="label text-success">Completed</span>';
                                                    } elseif ($order['appl_status'] == 2) { 
                                                        $status_value ='<span class="label text-success">Processed</span>';

                                                     } elseif ($order['appl_status'] == 3) { 
                                                        $status_value ='<span class="label text-success">Rejected</span>';

                                                    }
                                                    else
                                                    {
                                                        $status_value = '<span class="label text-danger" >Pending</ span>';
                                                    }
                                                    ?>
                                                    <td><?php echo $status_value; ?></td>
													<td>
                                                        <button type="button" class="btn btn-primary btn-sm waves-effect waves-light m-1 viewUser" data-toggle="modal" data-target="#largesizemodal" value="<?php echo $order['submit_id']; ?>">View</button>
														<!-- <?php if(has_admin_permission_layout('EDIT_USER')) { ?>
                                                        <a class="btn btn-warning btn-sm waves-effect waves-light m-1 edituser" data-toggle="modal" data-target="#largesizemodal" value="<?php echo $order['admin_id']; ?>" 
														href="<?php echo base_url() . 'user/add_edit_user/'. $order['submit_id'] ;?>"
														>Edit</a>
														<?php } ?> -->
